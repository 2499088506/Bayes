...
public class WholeInputFormat extends InputFormat<LongWritable, Text>{
			...
			throws IOException, InterruptedException 
	{
	    ...
	    long len = 0;
	    int index = 0;
//	    System.out.println("传入路径中的目录数: " + paths.length);
//	    for(Path pa : paths)
//	    {
...
//	    splits.add(new FileSplit(paths[0], 0, len, new String[0]));
		...
		for(Path pa : paths)
	    {
			...
			splits.add(new FileSplit(paths[index++], 0, len, new String[0]));
	    }
...
class MyInputRecordReader extends RecordReader<LongWritable, Text>
{
